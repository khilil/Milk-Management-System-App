...
const CustomerDetailScreen = ({ route }) => {
  ...
  const handlePrint = async () => {
    try {
      await print({
        html: `<h1>${customer.username}'s Milk Records</h1><table>${filteredRecords.map(r => `<tr><td>${r.date}</td><td>${r.quantity}</td><td>₹${parseFloat(r.quantity) * pricePerLiter}</td></tr>`).join('')}</table>`,
      });
    } catch (error) {
    ...
    <>
      ...
      <View style={styles.detailCard}>
        ...
        <View style={styles.infoRow}>
          ...
          <Text style={styles.infoText}>Daily Quantity: {customer.milkQuantity}</Text>
        </View>
        <View style={styles.divider} />
        <View style={styles.infoRow}>
